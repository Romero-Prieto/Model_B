...
text(xTab(1,1),1 - Sx(2)/F,nOTe{1},'Interpreter','latex','HorizontalAlignment','left','FontSize',10*z);
...
for i = 1:numel(y)
    if numel(ylab{i}) > 1
        text(r/D,y(i),ylab{i}{1},'Interpreter','latex','HorizontalAlignment','left','FontSize',11*z);
        text(r/D + 0.005/D,y(i) - 0.015/F,ylab{i}{2},'Interpreter','latex','HorizontalAlignment','left','FontSize',7.5*z);
    else
    ...
    if numel(y) > 5
        ...
        text(r/3/D,y(i),numeRaL,'Interpreter','latex','HorizontalAlignment','center','FontSize',5*z);
        text(1 - r/3/D,y(i),numeRaL,'Interpreter','latex','HorizontalAlignment','center','FontSize',5*z);
    end
end

for i = 1:numel(x)
    text(x(i),1 - Sx(2)/F,xlab{i},'Interpreter','latex','HorizontalAlignment','right','FontSize',11*z);
    for j = 1:numel(y)
        if isequal(W(j,i),0) 
            temp = [0.25 0.35 0.75];
        ...
        if ~isnan(data(j,3*i - 2))
            if isequal(floor(data(j,3*i - 2)),ceil(data(j,3*i - 2))) && data(j,3*i - 2) > 999              
                text(x(i),y(j),oBS(data(j,3*i - 2),foRMaT{1 + mod(numel(foRMaT) - 1 + i,numel(foRMaT))}),'Interpreter','latex','HorizontalAlignment','right','FontSize',11*z,'color',temp);
            else
                text(x(i),y(j),sprintf(foRMaT{1 + mod(numel(foRMaT) - 1 + i,numel(foRMaT))},data(j,3*i - 2)),'Interpreter','latex','HorizontalAlignment','right','FontSize',11*z,'color',temp);
            end
            
            if ~isequalwithequalnans(data(j,3*i - 1),data(j,3*i))
                text(x(i),y(j) - 0.015/F,char("$\mathit{[" + string(sprintf(foRMaT{1 + mod(numel(foRMaT) - 1 + i,numel(foRMaT))},data(j,3*i - 1))) + "," + string(sprintf(foRMaT{1 + mod(numel(foRMaT) - 1 + i,numel(foRMaT))},data(j,3*i))) + "]}$"),'Interpreter','latex','HorizontalAlignment','right','FontSize',7.5*z,'color',temp);
            end
        end
    end
end
