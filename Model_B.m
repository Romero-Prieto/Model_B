...
Beta{1}                 = {BetaA{3}(:,1:3:end),ReG};
Beta{2}                 = {BetaB{3}(:,1:3:end).*[1 1 1 -1],ReG};
...
for i = 1:numel(Beta)
    ...
    X          = Set(q,x,sET);
    X          = log(X{2}).^(0:Z);
    qm         = [zeros(1,size(q,2));exp(Beta{i}{1}(:,1:Z + 1)*X')];
    q          = Set(q,x,{F,G});
    qm         = Set(qm,x,{F,G});
    sCaTTeR{i} = q{2}./qm{2};
    clear q qm X
...
nMx                     = cell2mat(mat2cell(prctile(nMx(2:end,:),50)',ones(sum(sEL),1)*(numel(xE{1}) - 1))');
...
models            = {char("$\textrm{Log-quadratic model (N = " + size(Qm{4},2) + ")}$"),char("$\textrm{Model B (N = " + sum(sEL) + ")}$")};
...
group{4}          = {(1:size(Qm{4},2))'};
lEGeND{4}         = {char("$\textrm{Log-quadratic Model: U5MD (" + size(Qm{4},2) + ")}$")};
